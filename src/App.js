...
import { Upload, FileText, Clock, CheckCircle, XCircle, BarChart2, Repeat, Shuffle, Bot, Clipboard, Download, BookOpen, Pencil, History } from 'lucide-react';
...
const QuizTaker = ({ userId, appId }) => {
    ...
    const finishQuiz = React.useCallback(async () => {
        ...
        const finalResults = { score: score10.toFixed(1), correct: totalCorrect, incorrect: totalIncorrect, total: totalQuestions, incorrectQuestions, chartData, quizTitle: quizData.title, timestamp: serverTimestamp() };
    ...
    const handleFileUpload = (event) => { /* ... (sin cambios) ... */ };
    const startQuiz = () => { /* ... (sin cambios) ... */ };
    const handleAnswerSelect = (questionIndex, selectedOption) => { /* ... (sin cambios) ... */ };
    const handleNextQuestion = () => { /* ... (sin cambios) ... */ };
    const resetApp = () => { /* ... (sin cambios) ... */ };
    ...
    const QuizScreen = () => { /* ... (sin cambios) ... */ };
    const ResultsScreen = () => { /* ... (sin cambios) ... */ };
...
const QuizHistory = ({ userId, appId }) => {
    ...
    React.useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const historyData = snapshot.docs.map(doc => ({
                ...
                date: doc.data().timestamp?.toDate().toLocaleDateString('es-ES')
            })).sort((a, b) => b.timestamp - a.timestamp); // Ordena por fecha
    ...
    if (history.length === 0) {
        return <p className="text-center text-gray-400">No has completado ningún quiz todavía.</p>;
    }
